...
We’ve just highlighted the main differences in the interpreter and optimizing compiler pipelines for each JavaScript engine. But besides these differences, at a high level, _all JavaScript engines have the same architecture_: there’s a parser and some kind of interpreter/compiler pipeline.
...
_Note:_ The order in which properties are added impacts the shape. For example, `{ x: 4, y: 5 }` results in a different shape than `{ y: 5, x: 4 }`.
...
Even when just a single array element has non-default attributes, the entire array’s backing store goes into this slow and inefficient mode. _Avoid `Object.defineProperty` on array indices!_ (I’m not sure why you would even want to do this. It seems like a weird, non-useful thing to do.)
...
_Looking for more?_ If you’re interested in how JavaScript engines speed up accesses to prototype properties, read the next article in this series, [“JavaScript engine fundamentals: optimizing prototypes”](/2018/08/16/javascript-engine-fundamentals-optimizing-prototypes/).
